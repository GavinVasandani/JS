...
let create ~board ~snake =
  ...
  let allBoardLoc = Board.all_locations board in
  let allSnakeLoc = Snake.all_locations snake in
  let possibleLoc = List.filter allBoardLoc ~f:(fun boardPos -> not(List.mem allSnakeLoc boardPos ~equal:Position.equal)) in

  if List.is_empty possibleLoc 
  then None
  else let randomLoc = List.random_element_exn possibleLoc in
  Some {location = randomLoc}
;;
