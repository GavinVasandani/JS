...
let%expect_test "Exercise 03a" =
  ...
  in
  test (0, 0) true;
  [%expect {| in_bounds for ((height 10) (width 15)) ((col 0) (row 0)) returned false unexpectedly |}];
  test (-1, 0) false;
  [%expect {| |}];
  test (0, -1) false;
  [%expect {| |}];
  ...
  [%expect {| |}];
  test (9, 0) true;
  [%expect {| in_bounds for ((height 10) (width 15)) ((col 0) (row 9)) returned false unexpectedly |}];
  test (10, 0) false;
  [%expect {| |}];
  test (9, 14) true;
  [%expect {| |}];
