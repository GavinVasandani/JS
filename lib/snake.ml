...
let collides_with_self t =
  ...
  not(List.for_all t.tail ~f:(fun block -> not(Position.equal t.head block)))
...
let move_forward_and_grow ({ direction; extensions_remaining; head; tail } as t) =
  ...
  let new_position = Direction.next_position direction head in
  let post_tail = tail @ [head] in
  if extensions_remaining > 0
  then (t.extensions_remaining <- extensions_remaining - 1; new_position, post_tail)
  else new_position, List.tl_exn post_tail
