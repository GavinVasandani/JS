...
let handle_key t key =
  ...
  | None -> ()
  | Some d -> Snake.set_direction t.snake d;
;;
...
let check_for_collisions t = 
  ...
  if not (Board.in_bounds t.board (Snake.head t.snake)) then t.game_state <- Game_over "Out of bounds!"
 ;;
...
let maybe_consume_apple t = 
  ...
  let snake_head_loc = Snake.head t.snake in
  let apple_loc = Apple.location t.apple in
 
  if Position.equal snake_head_loc apple_loc 
